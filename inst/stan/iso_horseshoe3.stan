...
transformed parameters {
  ...
  theta[1] = 1.0 / sqrt((1.0 / 25.0) + (1.0 / (tau_sq[1] * lambda_sq[1])));
  if(n_groups_stan > 1) {
    for(i in 2:n_groups_stan) {
      theta[i] = 1.0 / sqrt((1.0 / 25.0) + (1.0 / (alpha_scale_stan_sq * tau_sq[i] * lambda_sq[i])));
    }
  }
  theta[n_groups_stan+1] = 1.0 / sqrt((1.0 / 25.0) + (1.0 / (tau_sq[n_groups_stan+1] * lambda_sq[n_groups_stan+1])));
